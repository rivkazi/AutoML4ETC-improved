...
epochs=get_conf("search.training_epoch_per_trial", 10)
...
class automl4etc():
    ...
    def search(self, train_dataset, test_dataset, input_shape, classes):
        ...
        mnist_data = np.load('C:/Users/zizovi/Documents/AutoML4ETC-main/mnist.npz')

        x_train1, y_train1 = mnist_data["x_train"], mnist_data["y_train"]
        x_test1, y_test1 = mnist_data["x_test"], mnist_data["y_test"]
    ...
    def quic_ucdavis_data_loader(self, path='./quic-dataset'):
        ...
        train_gen = commonio.datagen_separated.DataGenSeparated(samples, label_func, idxfilter=lambda x: (x % 10 < 8),
                                                                **kwargs)  # 80%
        test_gen = commonio.datagen_separated.DataGenSeparated(samples, label_func, idxfilter=lambda x: (x % 10 == 9),
                                                               **kwargs)  # 10%

        return train_gen, test_gen
    
    def quic_ucdavis_data_loader_validation(self, path='./quic-dataset'):
        NAMES = ["GoogleDoc", "GoogleDrive", "GoogleMusic", "GoogleSearch", "Youtube"]
        ...
        samples = [samples_address.replace('/home/orange/dataset-mat/quic/', path + '/quic-data/') for samples_address
                   in samples]
        ...
        validation_gen = commonio.datagen_separated.DataGenSeparated(samples, label_func,
                                                                     idxfilter=lambda x: (x % 10 == 8), **kwargs)  # 10%
