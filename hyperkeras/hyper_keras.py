...
#                 return initial_learning_rate * math.exp(-k*epoch)
        ...
        early_stopping=None
        configuration_dict =   configuration_dict = automl4etc_common._get_conf_dict()
        if(configuration_dict['search.child_model_early_stopping']):
            # Define early stopping callback with appropriate parameters
            early_stopping = EarlyStopping(monitor='loss', patience=5, verbose=1, restore_best_weights=True)
        
        if(configuration_dict['search.child_model_validation_data']):
            # Define validation data
            automl = automl4etc_common.automl4etc()
            validation_gen = automl.quic_ucdavis_data_loader_validation(path='./quic-dataset')
        ...
        if not validation_gen:
            if not early_stopping:
                ...
                print("--------------------------------- with early stopping  without validation ---------------------------")
                self.model.fit(X, callbacks=[LearningRateScheduler(lr_exp_decay, verbose=1),early_stopping], class_weight=class_weight, **kwargs)
        else:
            if not early_stopping:
                print("--------------------------------- with validation without early stopping---------------------------")
                self.model.fit(X, validation_data=validation_gen, callbacks=[LearningRateScheduler(lr_exp_decay, verbose=1)], class_weight=class_weight, **kwargs)
            else:
                print("--------------------------------- with validation and early stopping---------------------------")
                self.model.fit(X, validation_data=validation_gen, callbacks=[LearningRateScheduler(lr_exp_decay, verbose=1),early_stopping], class_weight=class_weight, **kwargs)
